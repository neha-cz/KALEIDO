...
5. Writes CSVs, PNG plots, and summary.json with strange-layer candidates and
   possible beta* / cliff behavior.
...
def detect_beta_star(beta_summary: pd.DataFrame) -> Dict[str, object]:
    ...
    for i in range(1, len(df)):
        drops.append(acc[i - 1] - acc[i])
    max_drop = max(drops)
    idx = drops.index(max_drop) + 1
    ...
    return {
        ...
        "drop_interval": [betas[idx - 1], betas[idx]],
        "interpretation": (
            "largest discrete accuracy cliff in the measured grid"
            if phase_like else
    ...
    if not args.skip_beta_sweep:
        ...
        for b in beta_values:
            ...
            df, ent = evaluate_condition(
                ...
                f"beta_{b:g}", args.max_new_tokens, args.metric_max_tokens
            )
            all_rows.append(df)
            all_entropy_rows.append(ent)
